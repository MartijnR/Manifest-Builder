...
When using the Code Igniter's PHP framework, this controller can simply be copied into the application/controllers folder. To test go to: yourdomain.com/manifest/html5 (or yourdomain.com/manifest/gears). If it works add the manifest url to your html.  If another framework is used you'll have to tweak/port the code (please create a fork).
